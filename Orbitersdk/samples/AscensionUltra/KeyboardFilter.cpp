...
KeyboardFilter::KeyboardFilter(void)
{
	...
	{
		OAPI2VK[i]=OAPI2VKtemp[i];
		VK2OAPI[i]=VK2OAPItemp[i];
	}
}
...
void KeyboardFilter::GetCheckString(char *msg)
{
	sprintf(msg, "GO ");
	short k, i;
	for(i=0;i<256;i++) if ((k=OAPI2VK[i])!=0) if (VK2OAPI[k]!=i) sprintf(msg, "%s%d>%d>%d ", msg, i, k, VK2OAPI[k]);
	for(i=0;i<256;i++) if ((k=VK2OAPI[i])!=0) if (OAPI2VK[k]!=i) sprintf(msg, "%s%d<%d<%d ", msg, i, k, OAPI2VK[k]);	
}
