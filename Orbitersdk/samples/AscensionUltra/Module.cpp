...
DLLCLBK VESSEL *ovcInit (OBJHANDLE hvessel, int flightmodel){return new AscensionUltra (hvessel, flightmodel);}

DLLCLBK void ovcExit (VESSEL *vessel){if (vessel) delete (AscensionUltra*)vessel;}

AscensionUltra *GetBase (HWND hDlg)
{
...
BOOL CALLBACK EdPg1Proc (HWND hTab, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_OLOCK_CLOSE:
			GetBase(hTab)->GetHangar(HANGARTYPETA, 0)->GetDoor(0)->Close();
			return TRUE;
		case IDC_OLOCK_OPEN:
			GetBase(hTab)->GetHangar(HANGARTYPETA, 0)->GetDoor(0)->Open();
			return TRUE;
		}
		break;
	}
...
BOOL CALLBACK EdPg3Proc (HWND hTab, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg) {
	case WM_INITDIALOG: {
		au = (AscensionUltra*)oapiGetVesselInterface ((OBJHANDLE)lParam);		
		} break;
	case WM_COMMAND:
		break;
	case WM_HSCROLL:
...
{
	AscensionUltra *au = (AscensionUltra*)oapiGetVesselInterface (hVessel);
...
{
	AscensionUltra *au = (uMsg == WM_INITDIALOG ? (AscensionUltra*)lParam : (AscensionUltra*)oapiGetDialogContext (hWnd));
	// pointer to vessel instance was passed as dialog context

	switch (uMsg) {
	case WM_INITDIALOG:
		UpdateCtrlDialog (au, hWnd);
		return FALSE;
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
		case IDCANCEL:
			...
			return TRUE;
		case IDC_OLOCK_CLOSE:
			au->GetHangar(HANGARTYPETA, 0)->GetDoor(0)->Close();
			return 0;
		case IDC_OLOCK_OPEN:
			au->GetHangar(HANGARTYPETA, 0)->GetDoor(0)->Open();
			return 0;
		}
		break;
	}
...
void UpdateCtrlDialog (AscensionUltra *au, HWND hWnd)
{
	...
	op = au->GetHangar(HANGARTYPETA, 0)->GetDoor(0)->GetPosition()==0.0?0:1;
