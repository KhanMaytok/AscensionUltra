...
namespace VirtualDockingTunnel
{
	...
	__declspec(dllexport) int Init(VESSEL *handle)
	{
	...
	__declspec(dllexport) int Exit(VESSEL *handle)
	{
		if (g_Handles.find(handle)==g_Handles.end()) return -1;
	...
	__declspec(dllexport) int SetDockState(VESSEL *handle, OBJHANDLE obj)
	{
		if (obj==NULL)
		{
	...
	__declspec(dllexport) float GetVersion(){return (float)0.1;}
